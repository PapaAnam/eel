...
window.scrollTo = (selector, tambahan = 0) => {
	let body = $("html, body");
	body.stop().animate({
		scrollTop : $(selector).position().top + tambahan
	}, 500, 'swing', function() { 
	});
}

window.macScrollTo = (selector, tambahan = 0) => {
	let body = $("html, .mac-content");
	body.stop().animate({
		scrollTop : $(selector).position().top + tambahan
	}, 500, 'swing', function() { 
	});
...
window.axios.defaults.baseURL = base_domain('/hris')
window.myaxios = axios.create({
	baseURL : base_domain('/api')
})
require('./hris/components')
import router from './hris/router'
...
window.handleError = (err, form) => {
	if(err.response.status == 409)
		...
		for(let key in errors){
			...
			console.log($(form).find('#'+key).length)
			...
			// datepicker
			$(form).find('#'+key).parents('.input-group').addClass('has-error').find('invalid-feedback').html(errors[key][0])
			$(form).find('#'+key).parents('.ddd').find('label').addClass('text-danger')
		}
